...
class CopyStage(BaseStage):
    ...
    def _build_prompt(
        ...
        return (
            ...
            "'**bold**'.\n"
            "- LinkedIn: a COMPLETE post of AT LEAST 600 words (up to 1200) — "
            "count them; hook-first; several substantive paragraphs; '->'/'>' "
            "Unicode bullets (no Markdown '-'/'*'); a closing question; 3-5 "
            "lowercase hashtags.\n"
            "- blog: a COMPLETE article of AT LEAST 1200 words (up to 2000) — "
            "count them; opens with the hook then a TL;DR block; a full narrative "
            "arc (problem -> constraint -> exploration -> solution -> result) "
            "across many paragraphs and sections; fenced code blocks with a "
            "language tag.\n\n"

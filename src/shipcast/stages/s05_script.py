...
class ScriptStage(BaseStage):
    ...
    def _build_prompt(brief_json: str, entry_json: str, context_json: str) -> str:
        ...
        return (
            ...
            "objects, each with image_prompt (str), narration (str), "
            "duration_sec (a number between 3 and 5 inclusive), and "
            "show_interface (bool). Set show_interface=true for beats that "
            "DEPICT THE PRODUCT'S UI / on-screen interface (so they can be "
            "grounded in the real app screenshot), and false for abstract, "
            "lifestyle, or hero shots that should not be forced to look like "
            "the website. Build on the brief's video_beats skeleton; open on "
            "the hook and close on a CTA.\n\n"
